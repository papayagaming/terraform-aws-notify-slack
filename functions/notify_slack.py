    ...
    return {
        ...
        "fields": [
            ...
            # },
                ...
                "title": "Reason",
                "value": f"```{json.dumps(json.loads(alarm_reason))}```",
                "short": False,
            } if relevant_info is {} else {}]+[{"title": k, "value": f"`{v}`", "short": True} for k, v in relevant_info.items()],
...
def get_log_for_alarm(alarm_name, namespace):
    ...
    result = logs.filter_log_events(
        ...
        startTime=int((datetime.today() - timedelta(minutes=60)).timestamp()),
